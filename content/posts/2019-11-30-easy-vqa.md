...
Quiz: what sport is depicted in this image?
...
It's time to start writing code! To begin, make sure you're using Python 3, then install a few packages:

```
pip install easy-vqa keras tensorflow
```

We'll be using the [easy-vqa](https://github.com/vzhou842/easy-VQA) Python package, which makes it simple to access the easy-VQA dataset.

To start, we'll just pull some data from `easy-vqa`:
...
Next, we'll read and pre-process our images for later use:
...
We'll take advantage of Keras's [Tokenizer](https://keras.io/preprocessing/text/) class to implement BOW:
