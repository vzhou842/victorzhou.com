...
Well, this post won't be like the others - instead of _telling_ you what it is, I'm going to **_show_** you... you'll see. Ready?
...
Notice also the "**Make Transfer**" button. That's a nice feature the Definitely Secure Bank website offers - you you can send money online with just one click! If you want to give it a go, try sending a dollar to test and see what happens:
...
Little do you know, there's no \$100 gift card (what?!?!) - in fact, **you've just lost $1,000.** Don't believe me? Go check your [Definitely Secure Bank](https://dsb.victorzhou.com/) tab (the one I told you to keep open for this post). It should look something like this:
...
We'll get to that in a second. First, let's take a step back: **that was a CSRF attack**. In short, CSRF is a web security exploit where an attacker induces a victim to **perform an action they didn't mean to.** In this case, the attacker tricked you into unintentionally transferring them money.
...
But how did this code manage to steal *your* money while including neither your username nor password? **Because you're logged in.** Browsers can send cookies to one origin (the DSB site) _even when the request originates from a different origin_ (the attacker's site).
