...
In reality, it's unlikely that our model learns exactly this kind of behavior. Remember that the only way the model learns is from [propagating gradients through the layers](/blog/intro-to-neural-networks/#4-training-a-neural-network-part-2), so whatever the image and question networks have learned to output are unlikely to look this simple. Just focus on the intuition that the "blue" information is embedded in the image vector and the "color" information in the question vector such that, when they're multiplied, the downstream layers can use them to output the right answer.
