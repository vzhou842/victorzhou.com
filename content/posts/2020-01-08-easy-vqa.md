...
In reality, it's very unlikely that our model learns exactly this kind of behavior. Remember that the only way the model learns is from [propagating gradients through the layers](http://victorzhou.com/blog/intro-to-neural-networks/#4-training-a-neural-network-part-2), so whatever the image and question networks have learned to output are unlikely to look as simple as, "first element in the image vector represents blue." But the intuition is that the "blue" information is embedded into the 32-dimensional image vector, and "color" in the question vector. And in ways such that when they're multiplied, the downstream layers can use them to output the right answer.
